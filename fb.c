    ...
    if (argc >= 1 && TYPE(argv[0]) == T_ARRAY) {
        ...
        if (RARRAY_LEN(ary) > 0 && TYPE(RARRAY_PTR(ary)[0]) == T_ARRAY) {
            ...
            for (i = 0; i < argc; i++) {
                ...
                isc_dsql_execute2(fb_connection->isc_status, &fb_connection->transact, &fb_cursor->stmt, SQLDA_VERSION1,
                                  fb_cursor->i_sqlda, NULL);
                fb_error_check(fb_connection->isc_status);
            }
        }
    } else {
...
static VALUE fb_cursor_fields_ary(XSQLDA *sqlda, short downcase_names) {
    ...
    for (count = 0; count < cols; count++) {
        ...
        field = rb_struct_new(rb_sFbField, name, sql_type, sql_subtype, display_size, internal_size, precision, scale,
                              nullable, type_code);
        ...
        if ((var->sqltype & 1) && (*var->sqlind < 0)) {
            ...
            switch (dtp) {
                ...
                case SQL_BLOB:
                    ...
                    isc_open_blob2(fb_connection->isc_status, &fb_connection->db, &fb_connection->transact,
                                   &blob_handle, &blob_id, 0, NULL);
                    fb_error_check(fb_connection->isc_status);
                    isc_blob_info(
...
static long cursor_rows_affected(struct FbCursor *fb_cursor, long statement_type) {
    ...
    switch (statement_type) {
        ...
        case isc_info_sql_stmt_delete:
            return deleted;
        default:
            return inserted + selected + updated + deleted;
    }
}
...
static VALUE cursor_execute2(VALUE args) {
    ...
    if (!fb_cursor->o_sqlda->sqld) {
        ...
        /* Open cursor */
        isc_dsql_execute2(fb_connection->isc_status, &fb_connection->transact, &fb_cursor->stmt, SQLDA_VERSION1,
                          in_params ? fb_cursor->i_sqlda : NULL, NULL);
        fb_error_check(fb_connection->isc_status);
...
//        VALUE print_str; //TODO print statement type
//        print_str = rb_sprintf("STATEMENT = %d", (int)statement);
//        rb_p(print_str);
        fb_connection_commit(fb_connection);

        fb_connection_transaction_start(fb_connection, statement == isc_info_sql_stmt_select ? read_transaction : Qnil); //read_only_transaction == 1 ? read_transaction : Qnil);
...
static VALUE connection_create(isc_db_handle handle, VALUE db) {
    ...
    int i;
    struct FbConnection *fb_connection;
    VALUE connection = Data_Make_Struct(rb_cFbConnection, struct FbConnection, fb_connection_mark, fb_connection_free,
                                        fb_connection);
...
static VALUE connection_columns(VALUE self, VALUE table_name) {
    ...
    for (i = 0; i < RARRAY_LEN(rs); i++) {
        ...
        column = rb_struct_new(rb_sFbColumn, name, domain, sql_type, sql_subtype, length, precision, scale, dflt,
                               nullable);
...
void Init_fb() {
    ...
    rb_sFbField = rb_struct_define("FbField", "name", "sql_type", "sql_subtype", "display_size", "internal_size",
                                   "precision", "scale", "nullable", "type_code", NULL);
    rb_sFbIndex = rb_struct_define("FbIndex", "table_name", "index_name", "unique", "descending", "columns", NULL);
    rb_sFbColumn = rb_struct_define("FbColumn", "name", "domain", "sql_type", "sql_subtype", "length", "precision",
                                    "scale", "default", "nullable", NULL);
